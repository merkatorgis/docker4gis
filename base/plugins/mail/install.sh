...
apk update; apk add --no-cache \
	mailx rsyslog postfix shadow bash
...
echo 'NOTICE: Run eg `addmailbox.sh user name` on image build or later, eg: `addmailbox.sh noreply "Example.com Template Service"`'
echo 'NOTICE: Run mail.sh to_address subject [from_user default noreply] to send mail.'
